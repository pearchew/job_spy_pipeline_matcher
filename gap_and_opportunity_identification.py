...
matched_master_path = Path("output") / "matched_master.csv"
today_str = datetime.now().strftime("%Y-%m-%d")
if not matched_master_path.exists():
    matched_master_path.write_text(
        "processed_date,keyword,company,title,date_posted,match_score,matched_skills,gaps_in_skill,job_url,description\n"
    )  # Create with headers
...
def evaluate_fit(description, candidate_profile):
    if pd.isna(description) or not str(description).strip():
        print("Skipping empty description...")
        return {"match_score": 0, "matched_skills": [], "gaps_in_skills": [], "matched_domain_expertise": [], "gaps_in_domain_expertise": []}
    extraction_prompt = f"""
...
for file_details in files_to_process:
    ...
    df["evaluation_json"] = df["description"].apply(lambda desc: evaluate_fit(desc, resume_content))
    ...
    df["matched_skills"] = df["evaluation_json"].apply(lambda x: ", ".join(x.get("matched_skills", [])))
    df["gaps_in_skill"] = df["evaluation_json"].apply(lambda x: ", ".join(x.get("gaps_in_skills", [])))
    df[
        [
            "processed_date",
            "keyword",
        ...
        matched_master_path, mode="a", index=False, header=not matched_master_path.exists()
    )
